...
class Admin_controller extends CI_Controller
{
    ...
    public function get_all_user()
    {
        ...
        $date = date("Y-m-d"); // la date a partir de laquelle la selection commence
        $first_date = date('Y-m-d', strtotime($date . '-10 days'));

        $date_end = '2022-01-01';
        $date1_end = str_replace('-', '/', $date_end);
        $second_date = date('Y-m-d', strtotime($date1_end . '+1 day')); // la date a partir de laquelle la selection finie
    ...
    public function update_event($id) {
        ...
        $data = array(
            ...
            // 'cat_event' => $this->event_model->join_categ($id),
            'session' => $this->session->userdata('username', 'role')
        );

        // var_dump($this->event_model->all_categ());
        // var_dump($this->event_model->join_categ($id));

        // $data['is_active'] = $this->get_all_catEventType($this->event_model->join_categ($id), $this->event_model->all_categ());
    ...
    private function get_all_catEventType($cat_event, $categories)
    {
        ...
        // $test = array_uintersect( $categories, $cat_event, function($categories, $cat_event){
        //     return spl_object_hash ($cat_event) <=> spl_object_hash ($categories);
        // });
        // return $test;
    }
